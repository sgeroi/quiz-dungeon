  ...
  return (
    <div className="h-full overflow-y-auto bg-gradient-to-b from-slate-950 via-purple-950/20 to-slate-950 text-white p-4">
      <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {q.options.map((opt, idx) => {
                ...
                const disabled = phase !== 'question' || myAnswer !== null;
                return (
                  <button
                    key={idx}
                    disabled={disabled}
        ...
        {/* Players list */}
        <div className="mt-6">
          <h3 className="text-sm uppercase tracking-wider text-gray-500 mb-2">Игроки</h3>
            ...
            {players.map(p => {
              ...
              return (
                <div key={p.id}
                  ...
                  <span className="truncate">{p.name}{p.isBot && ' 🤖'}</span>
                  {phase === 'question' && (
                  ...
                  {showCorrect && (
                    <span className={`text-xs ${isCorrect ? 'text-green-400' : 'text-rose-400'}`}>
                      {playerAnswer != null ? String.fromCharCode(65 + playerAnswer) : '—'}
                    </span>
                  )}
                </div>
              );
