...
async function apiGet<T>(url: string): Promise<{ data?: T; error?: string }> {
  try {
...
async function apiPost<T>(url: string, body: unknown): Promise<{ data?: T; error?: string }> {
  try {
...
async function apiDelete(url: string): Promise<{ error?: string }> {
  try {
...
export default function AdminScreen() {
  ...
  const deleteDungeon = async () => {
    ...
    if (!confirm('Удалить этот данжен?')) return;
  ...
  return (
    <div className="min-h-screen bg-[var(--color-dungeon-bg)]">
      <header className="sticky top-0 z-30 bg-[var(--color-dungeon-surface)] border-b border-gray-700/50 px-4 py-3 flex items-center gap-4">
        <a href="#/" className="text-gray-400 hover:text-white transition-colors text-sm">← Назад</a>
        <h1 className="text-lg font-bold text-[var(--color-dungeon-gold)]">⚙️ Конструктор данжена</h1>
      </header>

      <div className="flex flex-col lg:flex-row">
        <aside className="lg:w-72 shrink-0 border-b lg:border-b-0 lg:border-r border-gray-700/50 p-4">
          <button onClick={startNew} className="w-full py-2 px-4 rounded-xl bg-[var(--color-dungeon-accent)] text-white font-bold text-sm mb-4 hover:brightness-110 transition-all active:scale-[0.98]">
            + Создать новый
          </button>
          {apiError && (
            <div className="py-2 px-3 rounded-lg bg-red-900/40 border border-red-600/50 text-red-300 text-xs mb-3">{apiError}</div>
          )}
              ...
              <button
                ...
                className={`text-left p-3 rounded-xl border transition-all ${
                  editing?.id === d.id
                    ...
                    : 'bg-[var(--color-dungeon-surface)] border-gray-700 hover:border-gray-500'
                }`}
              >
                <div className="font-bold text-white text-sm truncate">{d.name || 'Без названия'}</div>
                <div className="text-xs text-gray-400 mt-1">{d.floors.length} этажей</div>
              </button>
            ))}
          </div>
        </aside>
        ...
        <main className="flex-1 p-4 lg:p-6 max-w-4xl">
          {!editing ? (
            <div className="flex items-center justify-center h-64 text-gray-500">Выберите данжен или создайте новый</div>
          ) : (
            <div className="flex flex-col gap-6">
              <section className="bg-[var(--color-dungeon-surface)] rounded-2xl p-4 border border-gray-700/50">
                <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wide mb-3">Основные настройки</h2>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <input type="text" placeholder="Название данжена" value={editing.name} onChange={e => updateField('name', e.target.value)} className="w-full py-2 px-3 rounded-xl bg-[var(--color-dungeon-bg)] border border-gray-700 text-white focus:border-[var(--color-dungeon-gold)] outline-none transition-colors" />
                  </div>
                  <div className="sm:col-span-2">
                    <input type="text" placeholder="Описание" value={editing.description} onChange={e => updateField('description', e.target.value)} className="w-full py-2 px-3 rounded-xl bg-[var(--color-dungeon-bg)] border border-gray-700 text-white focus:border-[var(--color-dungeon-gold)] outline-none transition-colors" />
                  </div>
                  <label className="flex items-center gap-2 text-sm text-gray-300">
                    ❤️ HP игроков
                    <input type="number" min={1} value={editing.personalHp} onChange={e => updateField('personalHp', Number(e.target.value))} className="w-20 py-1.5 px-2 rounded-lg bg-[var(--color-dungeon-bg)] border border-gray-700 text-white text-center" />
                  </label>
                </div>
              </section>

              <section>
                <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wide mb-2">Быстрый пресет</h2>
                <button onClick={applyDefaultPreset} className="px-3 py-1.5 rounded-lg bg-[var(--color-dungeon-surface)] border border-gray-600 text-gray-300 text-sm hover:border-[var(--color-dungeon-gold)] hover:text-white transition-all">
                  🏰 Стандартный (8 этажей)
                </button>
              </section>

              <section>
                <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wide mb-3">Этажи ({editing.floors.length})</h2>
                ...
                <button onClick={addFloor} className="mt-3 w-full py-3 rounded-xl border-2 border-dashed border-gray-600 text-gray-400 hover:border-[var(--color-dungeon-gold)] hover:text-[var(--color-dungeon-gold)] transition-all text-sm font-bold">
                  ➕ Добавить этаж
                </button>
              </section>

              <div className="flex gap-3 pb-8">
                <button onClick={saveDungeon} disabled={saving} className="flex-1 py-3 rounded-xl bg-[var(--color-dungeon-accent)] text-white font-bold hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-50">
                  {saving ? 'Сохранение...' : '💾 Сохранить'}
                </button>
                {editing.id && (
                  <button onClick={deleteDungeon} className="py-3 px-6 rounded-xl bg-red-900/40 border border-red-600/50 text-red-300 font-bold hover:bg-red-900/60 transition-all active:scale-[0.98]">
                    🗑️ Удалить
                  </button>
                )}
              </div>
